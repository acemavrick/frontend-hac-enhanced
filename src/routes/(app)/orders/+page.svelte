...
<div class="mx-auto max-w-3xl">
	<div class="flex items-center justify-between">
		<div>
			<h1 class="text-2xl font-bold text-text-primary">Scrape Orders</h1>
			<p class="mt-1 text-sm text-text-muted">View history and start new scrapes</p>
		</div>
		<NewScrapeButton />
	</div>
	...
	<div class="mt-6 space-y-4">
		{#if data.orders.length === 0}
			<div class="rounded-xl border border-dashed border-border bg-surface-raised p-12 text-center">
				...
				<h3 class="mt-3 text-sm font-medium text-text-primary">No orders yet</h3>
				<p class="mt-1 text-sm text-text-muted">Submit your first scrape to get started</p>
			</div>
		{:else}
