...
{#if completedOrders.length >= 2}
	<div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
		<h2 class="text-lg font-semibold text-gray-900">Compare Runs</h2>
		<p class="mt-1 text-sm text-gray-500">See what changed between two scrape runs</p>

		<div class="mt-4 flex flex-wrap items-end gap-3">
			<div>
				<label for="orderA" class="block text-xs font-medium text-gray-500">Older run</label>
				<select id="orderA" bind:value={orderA} class="mt-1 rounded-lg border-gray-300 text-sm shadow-sm focus:border-brand-500 focus:ring-brand-500">
			...
			<div>
				<label for="orderB" class="block text-xs font-medium text-gray-500">Newer run</label>
				<select id="orderB" bind:value={orderB} class="mt-1 rounded-lg border-gray-300 text-sm shadow-sm focus:border-brand-500 focus:ring-brand-500">
					<option value="">Select...</option>
					{#each completedOrders as o (o.id)}
						<option value={o.id}>{fmtDate(o.createdAt)} — {o.id.slice(0, 8)}</option>
					{/each}
		...
		{#if diff}
			<div class="mt-6 space-y-4">
				{#if diff.added.length === 0 && diff.removed.length === 0 && diff.changed.length === 0}
					<p class="text-sm text-gray-500">No differences found between these runs</p>
				{/if}

				{#if diff.added.length > 0}
					<div>
						<h3 class="text-sm font-medium text-green-700">Added ({diff.added.length})</h3>
				...
				{#if diff.removed.length > 0}
					<div>
						<h3 class="text-sm font-medium text-red-700">Removed ({diff.removed.length})</h3>
						<div class="mt-2 space-y-1">
				...
				{#if diff.changed.length > 0}
					<div>
						<h3 class="text-sm font-medium text-yellow-700">Changed ({diff.changed.length})</h3>
						<div class="mt-2 space-y-1">
							{#each diff.changed as item}
								<div class="flex items-center gap-2 text-sm">
									...
									<span class="text-gray-700">{item.after}</span>
								</div>
							{/each}
						</div>
					</div>
